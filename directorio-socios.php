...
<nav class="art-nav clearfix">
    <ul class="art-hmenu">
        ...
        <li><a href="directorio-socios.php" class="active">Directorio de Socios</a>
        	<ul>
				<?php
                ...
                <div class="art-content-layout">
                    <div class="art-content-layout-row">
                        <div class="art-layout-cell art-content clearfix"><article class="art-post art-article">
                                <h2 class="art-postheader"><span class="art-postheadericon">Directorio de Socios productores y comerciantes</span></h2><p><a href="files/Directorio_Socios.pdf" target="_blank">Descargar Archivo PDF</a></p>
                                                
                                <div class="art-postcontent art-postcontent-0 clearfix">

<?php
$localidades = consulta_bucle('SELECT DISTINCT localidad FROM registros');
while($localidad = mysql_fetch_array($localidades)){
	echo '<div class="art-content-layout-wrapper layout-item-0">
			<div class="art-content-layout layout-item-1">
				<div class="art-content-layout-row">
					<div class="art-layout-cell layout-item-2" style="width: 100%" >';
					echo '<a name="'. $localidad['localidad'] . '"><h2>Localidad: '. $localidad['localidad']. '</h2></a>';
					$consulta_gamep = 'SELECT DISTINCT gamep FROM registros WHERE localidad = "' . $localidad['localidad'] .'";';
					$gameps = consulta_bucle($consulta_gamep);
					while($gamep = mysql_fetch_array($gameps)){
						echo '<hr><h3>GAMEP: ' . $gamep['gamep']. '</h3>';
						$consulta_socio = 'SELECT * FROM registros WHERE localidad = "'. $localidad['localidad']. 
						'" AND gamep = "' . $gamep['gamep'] .'"';
						$socios = consulta_bucle($consulta_socio);
						while($socio = mysql_fetch_array($socios)){
							echo utf8_encode('<strong>Nombre del Socio:</strong> ' . $socio['nombre'] . '<br>');
							echo utf8_encode('<strong>Descripcion del Negocio:</strong> ' . $socio['descripcion_negocio'] . '<br>');
							echo utf8_encode('<strong>Domicilio:</strong> ' . $socio['domicilio'] . '<br>');
							if(isset($socio['telefono']) && !empty($socio['telefono'])){
								echo '<strong>Telefono:</strong> ' . $socio['telefono'] . '<br>';
							}//FIN if@while $socio
							echo '<br>';
						}//FIN while $socio@$gamep@$localidad
					}//FIN while $gamep@$localidad
				echo ' 		</div>
							</div>
						</div>
						</div>';
